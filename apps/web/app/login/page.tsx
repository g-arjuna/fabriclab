...
function FabricLabGlyph() {
  return (
    <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-400/20 bg-cyan-400/10 shadow-[0_0_30px_rgba(34,211,238,0.14)]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-12">
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <section className="rounded-[1.9rem] border border-white/10 bg-slate-900/70 p-6 sm:p-8">
            ...
            <div className="mt-8 rounded-[1.8rem] border border-cyan-400/10 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.12),transparent_48%),linear-gradient(180deg,rgba(8,18,35,0.94),rgba(4,10,22,0.96))] p-5 sm:p-6">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <div className="rounded-[1.5rem] border border-white/8 bg-[#020b16] p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">What unlocks</p>
                <p className="mt-2 text-sm leading-7 text-slate-300">
                  Published chapters, live labs, synced progress, and chapter discussion.
                </p>
              </div>
              <div className="rounded-[1.5rem] border border-white/8 bg-[#020b16] p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Return path</p>
                <p className="mt-2 text-sm leading-7 text-slate-300">
                  After auth, FabricLab sends you back to {nextPath}.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-[1.9rem] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-slate-950/40 sm:p-8">
            ...
            {!user ? (
              <div className={error ? "mt-6" : ""}>
                <p className="text-xs uppercase tracking-[0.28em] text-slate-500">Choose a provider</p>
                {providers.length > 0 ? (
                  <div className="mt-4 grid gap-3">
                    {providers.map((provider) => (
                      <Link
                        ...
                        className="flex items-center justify-between gap-4 rounded-[1.4rem] border border-white/10 bg-slate-950 px-4 py-4 text-left text-sm font-medium text-slate-100 transition hover:border-cyan-500/40 hover:bg-slate-900 hover:text-white"
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-100">
              ...
              <div className={`${error ? "mt-6 " : ""}rounded-[1.5rem] border border-cyan-400/30 bg-cyan-500/10 p-5`}>
