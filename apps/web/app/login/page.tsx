...
function LoginPageContent() {
  ...
  const supabaseEnv = useMemo(() => getPublicSupabaseEnv(), []);
  ...
  const callbackUrl = useMemo(() => {
    ...
    return `${supabaseEnv.appUrl}/auth/callback?next=${encodeURIComponent(nextPath)}`;
  }, [nextPath, supabaseEnv]);
  const [email, setEmail] = useState("");
  const [status, setStatus] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  if (!enabled && !supabaseEnv) {
    return (
      <main className="min-h-screen bg-[#020617] px-6 py-16 text-slate-100">
        <div className="mx-auto max-w-2xl rounded-3xl border border-white/10 bg-slate-900/70 p-8">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-400">
            Add the FabricLab auth environment variables to enable Google, GitHub, or magic-link sign-in.
          </p>
        </div>
      </main>
    );
          ...
          <div className="mt-8 space-y-6">
            <div className="rounded-2xl border border-white/8 bg-[#020b16] p-5">
              ...
              {providers.length > 0 ? (
                ...
                <p className="mt-4 text-sm leading-7 text-slate-400">
                  No OAuth providers are configured yet. You can still use a FabricLab magic link below.
                </p>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-white/8 bg-[#020b16] p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-400">
                We&apos;ll send a FabricLab sign-in email with a secure link back to this exact page.
              </p>
              <input
                id="email"
                type="email"
              ...
              <button
                type="button"
                disabled={!email || submitting || !callbackUrl}
                onClick={async () => {
                  const client = getBrowserSupabaseClient();
                  if (!client || !callbackUrl) {
                    setStatus("Magic-link sign-in is not configured yet.");
                    return;
                  }
                  ...
                  setStatus(
                    otpError
                      ? otpError.message
                      : `FabricLab sign-in link sent to ${email}. Open the email on this device to continue.`,
                  );
                }}
