...
The DGX H100 uses **4th-generation NVLink**, connected through **4 × 3rd-generation NVSwitch chips** inside the chassis. This gives each GPU **900 GB/s** of bidirectional bandwidth to communicate with all other GPUs in the same node — roughly 18 times faster than the external fabric connections we will see shortly.
...
- The entire tensor that was being transferred must be retransmitted from the beginning
- Every other GPU is sitting idle at the synchronisation barrier waiting
- At 400 Gb/s, "a tensor" might mean several gigabytes
- This happens thousands of times per hour during training
...
In a DGX H200, there are 2 BlueField-3 DPUs per node alongside 8 ConnectX-7 HCAs. The HCAs handle compute fabric RDMA. The DPUs handle storage and security — freeing the host CPUs to focus on managing the GPUs.
...
| 8× 3.84 TB NVMe U.2 (~30 TB) | Local OS, checkpointing — training data lives on shared parallel storage (BeeGFS, WEKA) |
...
Consider the math. A DGX SuperPOD has 32 nodes × 8 GPUs = 256 GPUs. A large training cluster might have 4,000 nodes × 8 GPUs = 32,000 GPUs. If each GPU has a mean time between failure (MTBF) of 10,000 hours — optimistic for a system under constant maximum load — then a cluster of 32,000 GPUs experiences a GPU failure every 18 minutes on average.
