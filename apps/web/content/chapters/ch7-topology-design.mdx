...
A single 400G switch with 64 ports can connect 64 DGX NICs directly. If each DGX node has 8 NICs, that is 8 nodes connected through one switch. During AllReduce, all 8 nodes send to all other nodes — 8 × 400G = 3.2 Tb/s of total traffic, all staying within one switch. No spine needed. Perfect bisection bandwidth. No bottlenecks.
...
The k-ary formula shows why switch radix matters so much: doubling radix from 32 to 64 does not double the cluster capacity — it multiplies it by 8 (2³). High-radix switches are disproportionately powerful for building large clusters.
...
- Switch hardware: Spectrum-X SN5600 (64 ports × 400G each)

**How the port math works**:
Each SN5600 has 64 ports. A leaf switch uses:
- 32 ports facing DGX nodes (4 DGX nodes × 8 NICs each? No — 32 nodes × 1 NIC per rail = 32 downlinks)
...
Where:
  Total downlink bandwidth = (number of switches in layer) × (downlinks per switch) × (link speed)
  Total uplink bandwidth   = (number of switches in layer) × (uplinks per switch) × (link speed)
```
...
Example:
  Leaf → Spine: 2:1 (each leaf switch has 2× more downlinks than uplinks)
  Spine → Core: 2:1 (each spine switch has 2× more downlinks than uplinks)
  Effective at bisection: 2:1 × 2:1 = 4:1
...
**2. Per-packet spraying headroom**: With more uplinks than downlinks, the leaf switch can spray each packet across a wider set of paths without any path becoming overloaded even in the worst case. The math works in your favour: if you have 2× as many uplinks as downlinks, the maximum load any spine link can receive during in-cast is 50% of its capacity — well below any PFC or ECN threshold.
...
```
Burst size = (number of nodes per rail) × (link speed) × (AllReduce synchronisation window)

Example: 32-node cluster, 400G links, 1ms synchronisation window
  Burst = 32 × 400 Gb/s × 0.001s = 16 Gb = 2 GB
...
**QSFP-DD (Quad Small Form-factor Double Density)**: 8 lanes × 50G PAM4 = 400G. The dominant connector for 400G Ethernet. Compatible with the QSFP28 form factor with an adapter. Used on most Spectrum-X switches.

**OSFP (Octal Small Form-factor Pluggable)**: Also 8 lanes × 50G PAM4 = 400G. Physically larger than QSFP-DD, with better thermal headroom. Used on some high-port-density switches and on the ConnectX-7 NIC in DGX H100/H200.
...
`Total NIC bandwidth = node count × NICs per node × NIC speed`

For 128 DGX H100 nodes: 128 × 8 × 400G = 409.6 Tb/s
...
- 128 nodes × 8 NICs = 1,024 NIC connections
- At 32 downlinks per leaf: 1,024 / 32 = 32 leaf switches
- 32 leaf switches × 32 uplinks = 1,024 uplinks to spine
- At 32 ports per spine switch (all uplinks): 1,024 / 32 = 32 spine switches
- This is a 2-stage design (32 leaf + 32 spine = 64 switches)

Check non-blocking:
- Each leaf: 32 downlinks × 400G = 12.8 Tb/s downlink
- Each leaf: 32 uplinks × 400G = 12.8 Tb/s uplink
- 1:1 → non-blocking ✓
...
32-node AllReduce burst per rail: 32 × 400G × 1ms = 1.6 Gb = 200 MB needed per switch
...
Leaf switches:    32 switches
  Downlinks:       16 ports × 400G to DGX nodes
  Uplinks:         16 ports × 400G to spine

Spine switches:  16 switches
  Downlinks:       32 ports × 400G to leaf
  Uplinks:         0 (2-stage only)

Wait — this requires 16 uplinks per leaf, but only 16 spine switches.
Each leaf sends 1 uplink to each spine. 32 leafs × 16 uplinks = 512 uplink connections.
16 spine switches × 32 ports = 512 connections. Matches. ✓

Oversubscription check:
  Leaf downlinks:   32 switches × 16 ports × 400G = 204.8 Tb/s
  Leaf uplinks:     32 switches × 16 ports × 400G = 204.8 Tb/s
...
Leaf switches:    8 switches (one per GPU rail)
  Downlinks:       32 ports × 400G to DGX nodes
  Uplinks:         32 ports × 400G to spine

Spine switches:  32 switches
  Downlinks:       8 ports × 400G to leaf
...
This configuration can only support 32 nodes, not 64. ✗
```

Proposal B as stated has an error — 8 leaf switches with 32 downlinks each gives only 256 NIC connections, insufficient for 64 nodes × 8 NICs = 512 connections. The vendor specified too few leaf switches for the stated node count. Correcting this to 16 leaf switches would provide the 512 connections needed. But then 16 leaf × 32 uplinks = 512 uplinks to spine, requiring 32 spine switches with 16 active ports each. With 64-port spine switches, 48 of 64 ports on every spine switch would be dark — 75% port waste. This is technically functional but economically irrational: you are paying for 64-port switch hardware while using fewer than 16 ports. The actual root failure of Proposal B is not switch radix — it is that the vendor quoted the wrong leaf count for the node target.
