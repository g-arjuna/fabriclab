...
3. **QP establishment**: Creates Queue Pairs between every GPU pair that will communicate across the external fabric — on a 32-node cluster this means creating QPs between 256 GPUs × 255 peers = 65,280 connections
...
**Communication volume**: Each GPU sends and receives exactly `2 × (N−1)/N × tensor_size` bytes. For large N this approaches `2 × tensor_size` — essentially constant regardless of GPU count. Every GPU is always sending and receiving simultaneously — near-perfect bandwidth utilisation.
...
Arranges GPUs in a binary tree hierarchy. Reduction happens in log₂(N) steps — dramatically fewer than ring's N−1.

For 256 GPUs: 8 steps instead of 255. Each step cuts the number of active senders in half. At the root, one GPU holds the complete sum and broadcasts it back down the tree in another log₂(N) steps.
...
Result: logarithmic step count (log₂(N) steps, like tree) AND near-optimal bandwidth utilisation (like ring). DBT is NCCL's default for large AllReduce operations and is the algorithm that makes SuperPOD-scale training practical.
...
# On a 32-node cluster: 32 × 31 × 4 = 3,968 active flows across 8 rails
...
**`size`**: Message size in bytes (this is what one GPU is contributing — the total AllReduce processes N × this).
...
**`busbw`** (bus bandwidth): `algbw × 2(N−1)/N`. This is the number that matters. It corrects algbw by the mathematical efficiency of the ring algorithm, giving you the effective utilisation of the NIC bandwidth. At large N, busbw ≈ 2 × algbw. This is what you compare against line rate.
...
busbw of 138 GB/s on a 128-GPU cluster (16 nodes × 8 GPUs) is within expected range for this topology. `#wrong: 0`.
