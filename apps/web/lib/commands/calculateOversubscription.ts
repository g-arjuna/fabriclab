...
export function calculateOversubscriptionA(): CommandResult {
  const { setCondition, markVerified, setTopology, topology } =
    useLabStore.getState();

  setTopology({
    ...(topology as object),
    proposalACalculated: true,
  } as typeof topology);
...
Bisection bandwidth check:
  ...
  Each half gets 102.4 Tb/s -> full bisection bandwidth met OK
...
Port utilisation:
  DGX-facing leaf ports available: 256 / 512 required = 50% of required host capacity
  Spine ports used:                32 spines x 8 leaf-facing links = 256
  Spine ports provisioned:         32 spines x 64 ports = 2048
  Spine ports idle in this BOM:    1792 / 2048 = 87.5% idle
...
Verdict: Proposal B fails to meet the 64-node requirement.
         Insufficient leaf switches. 87.5% of the purchased spine ports sit idle in this BOM.
         Economically irrational - higher cost per connected node than Proposal A.`,
    conceptId: "rocev2",
    type: "error",
  };
}
