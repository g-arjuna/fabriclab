...
export function EthtoolOutputViz() {
  ...
  return (
    <div className="my-8 rounded-2xl border border-white/10 bg-slate-900 p-5">
      ...
      <div className="rounded-xl bg-[#0a0f1a] border border-white/10 p-4 font-mono text-xs leading-7 text-slate-300 mb-4">
        <div className="text-slate-500">NIC statistics:</div>
        {counters.map((c, i) => (
          <button key={i} onClick={() => setActive(active === i ? null : i)}
            className="w-full text-left pl-4 rounded hover:bg-slate-800/50 transition-all flex items-center gap-2">
